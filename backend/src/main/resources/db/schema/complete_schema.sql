...
-- Transfer system, consolidated from all Flyway migration files.
-- 
-- Generated from migrations V001 through V030
-- Excludes transactional data and seed data
-- =====================================================================
...
-- Flow deployment log for tracking deployment history
...
-- Function for system configuration timestamp updates
...
-- Function for PGP keys timestamp updates
CREATE OR REPLACE FUNCTION update_pgp_keys_timestamp()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = CURRENT_TIMESTAMP;
    RETURN NEW;
...
-- Function to check if a flow can execute
CREATE OR REPLACE FUNCTION can_flow_execute(flow_id UUID)
RETURNS BOOLEAN AS $$
DECLARE
    flow_active BOOLEAN;
    sender_active BOOLEAN := false;
    receiver_active BOOLEAN := false;
    sender_started BOOLEAN := false;
    receiver_started BOOLEAN := false;
    sender_adapter_id UUID;
    receiver_adapter_id UUID;
BEGIN
    -- Check if the flow exists and is active
    SELECT active INTO flow_active 
    FROM integration_flows 
    WHERE id = flow_id;
    
    IF flow_active IS NULL OR NOT flow_active THEN
        RETURN false;
    END IF;
    
    -- Check deployed flow status and get adapter IDs
    SELECT df.sender_adapter_id, df.receiver_adapter_id
    INTO sender_adapter_id, receiver_adapter_id
    FROM deployed_flows df
    WHERE df.flow_id = can_flow_execute.flow_id 
      AND df.deployment_status = 'DEPLOYED'
      AND df.execution_enabled = true
      AND df.runtime_status = 'ACTIVE';
    
    -- If no active deployment found
    IF sender_adapter_id IS NULL THEN
        RETURN false;
    END IF;
    
    -- Check sender adapter status
    SELECT active, (status = 'STARTED') 
    INTO sender_active, sender_started
    FROM adapters 
    WHERE id = sender_adapter_id;
    
    IF NOT sender_active OR NOT sender_started THEN
        RETURN false;
    END IF;
    
    -- Check receiver adapter status (if exists)
    IF receiver_adapter_id IS NOT NULL THEN
        SELECT active, (status = 'STARTED') 
        INTO receiver_active, receiver_started
        FROM adapters 
        WHERE id = receiver_adapter_id;
        
        IF NOT receiver_active OR NOT receiver_started THEN
            RETURN false;
        END IF;
    END IF;
...
-- Function to extract adapter references from flow definitions
CREATE OR REPLACE FUNCTION extract_flow_adapters(flow_def JSONB)
RETURNS JSONB AS $$
DECLARE
    result JSONB := '{"sender_adapters": [], "receiver_adapters": []}';
    node JSONB;
    node_type TEXT;
    adapter_ref TEXT;
BEGIN
    -- Extract adapter references from flow nodes
    FOR node IN SELECT jsonb_array_elements(flow_def->'nodes')
    LOOP
        node_type := node->>'type';
        
        IF node_type = 'sender_adapter' THEN
            adapter_ref := node->>'adapterId';
            IF adapter_ref IS NOT NULL THEN
                result := jsonb_set(
                    result, 
                    '{sender_adapters}', 
                    (result->'sender_adapters') || jsonb_build_array(adapter_ref)
                );
            END IF;
        ELSIF node_type = 'receiver_adapter' THEN
            adapter_ref := node->>'adapterId';
            IF adapter_ref IS NOT NULL THEN
                result := jsonb_set(
                    result, 
                    '{receiver_adapters}', 
                    (result->'receiver_adapters') || jsonb_build_array(adapter_ref)
                );
            END IF;
        END IF;
    END LOOP;
...
-- Function for updating flow adapter references
CREATE OR REPLACE FUNCTION update_flow_adapter_references()
RETURNS TRIGGER AS $$
BEGIN
    -- Extract and store adapter references when flow definition changes
    NEW.updated_at = CURRENT_TIMESTAMP;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
CREATE TRIGGER trigger_update_system_configuration_timestamp
    BEFORE UPDATE ON system_configuration
    FOR EACH ROW
    EXECUTE FUNCTION update_configuration_timestamp();

CREATE TRIGGER trigger_update_pgp_keys_timestamp
    BEFORE UPDATE ON pgp_keys
    FOR EACH ROW
    EXECUTE FUNCTION update_pgp_keys_timestamp();

CREATE TRIGGER trigger_update_adapters_timestamp
    BEFORE UPDATE ON adapters
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER trigger_update_integration_flows_timestamp
    BEFORE UPDATE ON integration_flows
    FOR EACH ROW
    EXECUTE FUNCTION update_flow_adapter_references();

CREATE TRIGGER trigger_update_deployed_flows_timestamp
    BEFORE UPDATE ON deployed_flows
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();
...
-- Grant permissions to application user (assuming h2h_user exists)
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO h2h_user;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO h2h_user;
GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA public TO h2h_user;

-- =====================================================================
-- SCHEMA COMPLETE
-- =====================================================================
-- This schema provides a complete foundation for the H2H File Transfer
